    ...
    <div className="mx-auto max-w-6xl space-y-4">
      ...
      <fieldset disabled={!canEdit || saving} className="space-y-4" onChange={() => setSaved(false)} onClickCapture={() => setSaved(false)}>
        ...
        {canEdit && <details className="rounded-2xl bg-white p-4">
          ...
          <p className="mb-3 text-sm">Photograph the full, flat 2027 TIPS sheet in good light. The app reads handwriting from its known boxes, then lets you correct every suggested value before applying it. The photo is sent to the OCR service but is not stored with this report. The overall decision and signature need your own review.</p>
          <label className="block"><span className="mwos-form-label">TIPS sheet photo</span><input type="file" accept="image/*" disabled={scanning} onChange={e => {setFile(e.target.files?.[0] || null);updateScanText('');setScanError('');}} /></label>
          {photoPreviewUrl && <div className="mt-3 rounded-xl border border-[var(--color-primary-border)] p-2"><p className="mb-2 text-xs font-bold uppercase text-[var(--color-mid)]">Original photo for checking OCR values</p><img src={photoPreviewUrl} alt="Selected TIPS sheet" className="max-h-[28rem] w-full object-contain" /></div>}
