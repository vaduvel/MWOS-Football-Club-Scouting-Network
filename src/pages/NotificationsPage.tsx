  ...
  return (
    <article className={`mwos-subcard ${tone} p-4 md:p-5`}>
      <div className="mwos-subcard-head">
        ...
        <p className="mwos-subcard-value mt-0 min-h-9 tabular-nums md:text-4xl" aria-busy={loading}>
...
export default function NotificationsPage() {
  ...
  async function refresh() {
    ...
    try {
      ...
      const roleVisibleItems = data.items.filter((item) => {
        const category = getNotificationCategory(item);
    ...
    <div className="min-h-dvh bg-[var(--color-light)] md:flex">
      ...
      <main className="flex-1 overflow-auto px-4 pb-28 pt-[calc(env(safe-area-inset-top)+1rem)] md:p-6">
        <div className="mx-auto max-w-7xl space-y-6">
                ...
                <div className="mwos-surface-intro-copy">
                  ...
                  <p className="mwos-hero-copy mt-2.5 max-w-3xl text-pretty text-white/82 md:mt-3">
                    {canSeeOperationalAlerts
                      ? 'Keep internal announcements and operational notifications in one mobile-friendly feed, then open the relevant workspace when action is needed.'
                      : 'Read club announcements relevant to your scouting work in one clear, mobile-friendly feed.'}
                  </p>
                </div>
              </div>
            </div>
          ...
          <section className={cn('grid grid-cols-2 gap-3 sm:gap-4', canSeeOperationalAlerts ? 'xl:grid-cols-4' : 'xl:grid-cols-3')}>
            <StatCard loading={loading} label="All Alerts" mobileLabel="All" value={stats.total} detail="Everything currently visible in the alerts workspace." tone="mwos-subcard-report" icon={Bell} />
            <StatCard loading={loading} label="Unread" value={stats.unread} detail="Announcements or updates that still need attention." tone="mwos-subcard-alert" icon={Mail} />
            <StatCard loading={loading} label="Announcements" mobileLabel="Posts" value={stats.announcements} detail="Internal posts shared with the club or selected teams." tone="mwos-subcard-staff" icon={Megaphone} />
            {canSeeOperationalAlerts ? <StatCard loading={loading} label="Operational" mobileLabel="Ops" value={stats.notifications} detail="Operational notifications available to your assigned role." tone="mwos-subcard-training" icon={Send} /> : null}
          </section>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-2 max-w-2xl text-sm font-semibold leading-7 text-[var(--color-mid)]">
                  {canSeeOperationalAlerts ? 'Move between announcements and the operational notifications available to your role.' : 'Filter the club announcements available to your Scout account.'}
                </p>
              </div>
          ...
          <section className="rounded-[28px] border border-[var(--color-mid)]/16 bg-white p-6 shadow-[0_18px_45px_rgba(49,39,131,0.06)]">
            <div className="mwos-surface-intro">
              ...
              <div className="mwos-surface-intro-copy">
                ...
                <p className="mt-2 text-sm font-semibold leading-7 text-[var(--color-mid)]">
                  {canSeeOperationalAlerts
                    ? 'Internal announcements stay visible here alongside the operational updates available to your assigned role.'
                    : 'Club announcements for your Scout account stay visible here until they expire or are archived.'}
                </p>
              </div>
            </div>
