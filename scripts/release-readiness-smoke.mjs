...
export async function runReleaseReadinessSmoke({ email, password }) {
  if (!email || !password) {
  ...
  const launchReadiness = buildLaunchReadiness({
    publicAppUrl: appRuntimeStatus.publicAppUrl || publicAppUrl,
    footballApiProvider,
    footballApiKey,
    adminAiStatus,
    adminEmailStatus,
...
if (isDirectRun) {
  ...
  const result = await runReleaseReadinessSmoke({ email, password });
